...
# @date   2011/08/20
# 
# Add anchor coderay-0.9.8/lib/coderay/encoders/html.rb .
...
module Encoders
  ...
  #
  # Default: false
  class HTMLAnchor < Encoder
    include Streamable
    register_for :html_anchor
    ...
    # for vendor/
    # helper :output, :css
    require File.join(File.dirname(__FILE__), 'htmlanchor/output')
    require File.join(File.dirname(__FILE__), 'htmlanchor/css')
  ...
  protected
      ...
      when :class
        @css_style = Hash.new do |h, k|
          ...
          if c == :NO_HIGHLIGHT and not hint
            ...
            title = if hint
              HTMLAnchor.token_path_to_hint(hint, k[1..-1] << k.first)
            else
              ''
            end
            if c == :NO_HIGHLIGHT
      ...
      when :style
        @css_style = Hash.new do |h, k|
          ...
          if classes.first == :NO_HIGHLIGHT and not hint
            ...
            title = HTMLAnchor.token_path_to_hint hint, styles
            style = @css[*classes]
            h[k] =
              if style
    ...
    def finish options
      ...
      anchor_str = create_anchor @out, options
    ...
    def create_anchor(str, options)
      if options[:highlight_lines]
        lines = str.split("\n")

        codes = options[:highlight_lines].map {|no|
